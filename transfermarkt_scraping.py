...
TEAMS_FILE_PATH = "teams_urls.txt"
PLAYERS_FILE_PATH = "players_urls.txt"
DATASET_FILE_PATH = "transfermarkt_dataset.txt"
...
def iterate_through_countries(countries, urls_file_path, countries_dict_file_path):
    """
    ...
    save_to_file(list_of_countries_urls, urls_file_path)
...
def get_players_data(players_urls, request_headers, file_path, num_of_players, restart_from_beginning=True):
    """
    ...
    for player in players_urls:
        ...
        if counter_for_checkpoint % 250 == 0:
            append_to_tsv(players_data, file_path)
            players_data = []
            print('Counter: {}/{}'.format(counter_for_checkpoint, num_of_players))
            time2 = time.time()
            time_per_batch = time2 - time1
            print('Time to finish: {}/{}'.format(str(time_per_batch * (counter_for_checkpoint // 250)), \
                  str(time_per_batch * (num_of_players // 250))))
            time1 = time2
...
def start_scraping(request_headers, countries, tiers, file_paths):
    countries_urls = iterate_through_countries(countries, file_paths[0], file_paths[5])
    ...
    num_of_players = len(players_urls)
    get_players_data(players_urls, request_headers, file_paths[4], num_of_players)
